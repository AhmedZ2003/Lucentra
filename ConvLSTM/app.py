...
import yaml
from get_speeds import get_speed_ema, get_speed_ema_gaussian_smoothing, get_speed_gaussian_running_avg_smoothing, get_speed_gaussian_smoothing, get_speed_running_avg
...
def analyze_video():
    ...
    try:
        ...
        try:
            ...
            # speeds = get_speed_ema(model, temp_path, device, transform, alpha=0.1)
            # speeds = get_speed_ema_gaussian_smoothing(model, temp_path, device, transform, alpha=0.1, smooth_sigma=9.0)
            speeds = get_speed_gaussian_smoothing(model, temp_path, device, transform, smooth_sigma=5.0)
            # speeds = get_speed_gaussian_running_avg_smoothing(model, temp_path, device, transform, avg_ratio=0.4, smooth_sigma=5.0)
            # speeds = get_speed_running_avg(model, temp_path, device, transform, avg_ratio=0.4)
